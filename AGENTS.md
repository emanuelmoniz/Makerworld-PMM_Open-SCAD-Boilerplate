...
| Lint + smoke check | `scripts\check.bat` | `bash scripts/check/check.sh` |
...
standalone. There is no package manager. The test suite is `scripts/check/` (lint + smoke).
...
2. **Run the checks**: `scripts/check/check.sh`. A lint ERROR or smoke failure is a blocker. Fix
   it or report it; don't leave it silently.
3. **Update both READMEs** with whatever is newly relevant: `README.md` (keep the
   ...
   `dist/makerworld_listing.md`. The listing's changelog is **customer-facing only**: features,
