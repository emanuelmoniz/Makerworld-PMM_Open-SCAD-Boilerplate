@echo off
REM ============================================================
REM <PROJECT NAME>  |  Render preview PNGs per scripts\render_config.sh (manual only)
REM Double-click, or run from a terminal. Optional first argument: a project
REM folder, e.g.  render.bat examples\demo   (default: this repo's root project).
...
"%BASH_EXE%" "%~dp0render/render.sh" %PROJECT_ARG%
if errorlevel 1 (
    echo.
